...
class NumerovSolver {
    ...
    public:
        ...
        NumerovSolver(double dr_val = 0.01, double r_max_val = 20.0) 
            : dr(dr_val), r_max(r_max_val), n_points(static_cast<int>(r_max / dr)) {}

        double solveAndExtractPhase(int l, double k, const Potential& pot,
                                    double Z1, double Z2) {
            std::vector<double> yl(n_points);   // radial solution to the Schroedinger's equation
            ...
            std::vector<double> k_sq(n_points);
            for (int i = 0; i < n_points; ++i) {
                const double E = (k*k * constant::HBARC*constant::HBARC) / (2.0 * mu);
                k_sq[i] = - 2.* mu/(constant::HBARC*constant::HBARC) * V[i] - l*(l+1)/(r[i]*r[i]) + 2.*mu*E/(constant::HBARC*constant::HBARC);
            }
            ...
            double h2_12 = dr * dr / 12.0;
            for (int i = 1; i < n_points - 1; ++i) {
                double f0 = 1.0 + h2_12 * k_sq[i - 1];
                double f1 = 1.0 - 5.0 * h2_12 * k_sq[i];
                double f2 = 1.0 + h2_12 * k_sq[i + 1];
            ...
            const int idx0 = n_points - 3;
            const int idx1 = n_points - 2;
            const int idx2 = n_points - 1;

            // Match to asymptotic form: yl ~ A sin(kr - l*pi/2 + delta)
            const double dyl_dr = (yl[idx1] - yl[idx0]) / dr;
            const double log_deriv = dyl_dr / yl[idx2];
            double delta = TMath::ATan2(k, log_deriv) + l*TMath::Pi()/2. - k*r[idx2];
...
void chi2Function(Int_t &npar, Double_t *gin, Double_t &f, Double_t *par, Int_t iflag) {
    ...
    double chi2 = 0.0;
    double mu = reducedMass(constant::M_PROTON, constant::M_HE3);
    NumerovSolver solver(0.005, 30.0);
    ...
    for (size_t i = 0; i < g_data.l_values.size(); ++i) {
        ...
        double calc_phase = solver.solveAndExtractPhase(g_data.l_values[i], k, *g_potential, constant::Z_PROTON, constant::Z_HE3);
...
void chi2FunctionRegularization(Int_t &npar, Double_t *gin, Double_t &f, Double_t *par, Int_t iflag) {
    ...
    calc_phases.reserve(g_data.l_values.size());
    ...
    for (size_t i = 0; i < g_data.l_values.size(); ++i) {
        ...
        double calc_phase = solver.solveAndExtractPhase(g_data.l_values[i], k, 
