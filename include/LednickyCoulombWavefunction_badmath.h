...
std::complex<double> LednickyCoulombWavefunction::G_function(
    ...
    std::complex<double> result = std::sqrt(A_C) * 
...
void LednickyCoulombWavefunction::wavefunction_at_point(
    ...
    std::complex<double> phase(std::cos(arg.val), std::sin(arg.val));
    
    // Wavefunctions
    std::complex<double> common = phase * std::sqrt(A_C) * 
                                  std::exp(std::complex<double>(0.0, -rho)) * F;
    
    psi_s = common + phase * std::sqrt(A_C) * f_c_s * G / r;
    psi_t = common + phase * std::sqrt(A_C) * f_c_t * G / r;
}
...
double LednickyCoulombWavefunction::correlation_function_optimized(
    ...
    double source_norm = 1.0 / TMath::Power(2.0 * R_source * std::sqrt(TMath::Pi()), 3);
