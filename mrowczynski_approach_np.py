...
#                   * erf(2. * np.sqrt(6) * 1j * Rsource * k/ 3))
#
...
def correlation_function_strong(Rsource, a0, k):
    ...
    integrand = lambda r: np.exp(-(3*r**2)/(8*Rsource**2)) * np.sin(2*k*r)
    integral, _ = quad(integrand, 0, 0.5)
    
    term1 = 1 + (3/(4 * Rsource**2)) * f*np.conj(f)
    term2 = -3 * (1 - np.exp(-(8/3) * Rsource**2 * k**2)) / (4 * Rsource**2 * k) * np.imag(f)
    term3 = (np.sqrt(27)*np.real(f)) / (np.sqrt(32 * np.pi) * Rsource**3 * k) * integral
...
def correlation_function_CATS(Rsource, a0, r0, k, eta):
    ...
    h_eta = 0.5 * (digamma(1j*eta) + digamma(-1j*eta) - np.log(eta*eta))
    inv_f = -1./a0 + 0.5*r0*k**2 - 1j*Ac*k - 2.*k*eta*h_eta
    f = 1./inv_f
    Dawson_arg = 2*k*Rsource
    Dawson = quad(lambda x: np.exp(-Dawson_arg**2+x**2), 0, Dawson_arg)[0]

    Ck = Ac * ( 1 + 0.5*f*np.conj(f)/(Rsource*Rsource) * (1 - r0/(2*np.pi*Rsource) + 0.5*(Ac - 1)**2*(1. - np.exp(-4*k*k*Rsource*Rsource)) )
               +  np.real(f)*Dawson/(np.sqrt(np.pi)*k*Rsource*Rsource) - np.imag(f)*(1 - np.exp(-Dawson_arg*Dawson_arg))/(2.*k*Rsource*Rsource)
               + 2.*(Ac - 1.)*np.cos(k*Rsource)*np.exp(-k*k*Rsource*Rsource) )
...
def correlation_function_paper(Rsource, a0, r0, k, eta):
    ...
    Ck = Ac * ( 1 + 0.5*f*np.conj(f)/(Rsource*Rsource) * (1 + 0.5*(Ac**2 - 1)*(1. - np.exp(-4*k*k*Rsource*Rsource)) )
               +  np.real(f)*Dawson/(np.sqrt(np.pi)*k*Rsource*Rsource) - np.imag(f)*(1 - np.exp(-Dawson_arg*Dawson_arg))/(2.*k*Rsource*Rsource) )
...
def plot_strong_interaction_Coulomb(outfile:TFile, mass1, mass2, z1, z2, a0s, a0t, clebsch_gordan_coef=(0.25, 0.75)):
    ...
    for radius in [1.5, 1.73, 2., 2.5, 3., 5.08, 6.23, 7, 7.19]:
        ...
        for k in np.arange(kmin, kmax, kstep):
            ...
            h_Cks_strong.SetBinContent(h_Ck_strong.FindBin(k), np.real(Cks))
            h_Ckt_strong.SetBinContent(h_Ck_strong.FindBin(k), np.real(Ckt))
            h_Ck_strong.SetBinContent(h_Ck_strong.FindBin(k), np.real(Ck))
            h_Ck_Coulomb.SetBinContent(h_Ck_Coulomb.FindBin(k), Ac)
            h_Ck_Coulomb_swave.SetBinContent(h_Ck_Coulomb_swave.FindBin(k), Ac*np.real(Ck))
            ...
            h_Ck_Coulomb_swave_CATS.SetBinContent(h_Ck_Coulomb_swave_CATS.FindBin(k), np.real(Ck_CATS))
            ...
            h_Ck_Coulomb_swave_paper.SetBinContent(h_Ck_Coulomb_swave_paper.FindBin(k), np.real(Ck_paper))
...
def compare_strong_interaction_Coulomb(outfile:TFile, pdf_path:str,  **kwargs):
    ...
    legend.Draw()
    canvas.SaveAs(f'{pdf_path}/compare_Coulomb_swave_np.pdf')
...
if __name__ == '__main__':
    ...
    if do_computation:
        outfile = TFile.Open('output/mrowczynski_approach_pHe3_np.root', 'recreate')
